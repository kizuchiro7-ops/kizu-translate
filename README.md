...
### 久しぶりに開いた最初の1回だけ失敗する（2026-08-27に報告・対処済み）

2日ぶりに起動した際、会話モードの「お話しください」画面でエラーが出て読み上げも
鳴らない、数分後には正常、という報告があった。原因は2つ。

1. **音声合成の解錠が失われる** — iOSはユーザー操作まで `speechSynthesis` を鳴らさない。
   `primeSpeech()` で最初のタップに解錠していたが、**一度きり**だったため、アプリを長時間
   バックグラウンドに置いて復帰すると解錠が失われたまま無音になっていた。
   `visibilitychange` / `pageshow(persisted)` / `focus` の3経路で待ち受けを張り直すようにした
   （同じ関数参照なので `addEventListener` は重複登録しない）。復帰時に `cancel()` も呼ぶ
   ——iOSは復帰直後 `speaking` が立ちっぱなしになり、以降の `speak()` を黙って捨てることがある。
2. **聞き取りは通信必須** — iOS/Androidとも音声をサーバーに送って文字にするため、
   スリープ復帰直後のWi-Fi再接続中は `network` で落ちる。`network` と不明なエラーは
   **自動で1度だけ入れ直す**ようにし（オーバーレイは「接続し直しています…」のまま維持）、
   それでも駄目なら原因の分かる文言を出す。

あわせて、会話モードとAI翻訳タブのマイクが**別インスタンスで衝突**していた問題も修正した
（`stopAllRecognition()` で起動前に両方止める）。`rec.start()` の例外時も1度だけやり直す。
